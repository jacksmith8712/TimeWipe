...
{
	return (CGPoint){view.center.x, 36};
};

CGPoint DownPointOfView(UIView *view)
{
	return (CGPoint){view.center.x, 56};
};
...
-(void)touchesEnded:(NSSet*)touches  withEvent:(UIEvent*)event
{
	...
	{
		[self deactivate:activeView];
        if ([_delegate respondsToSelector:@selector(tapOnView:)]) {
            [_delegate performSelector:@selector(tapOnView:) withObject:activeView];
        }
		activeView = nil;
	}
}
